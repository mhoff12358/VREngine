...
function create_scene ()
	...
	function scene_table.initialize(self)
		...
		local right_laser = self:add_actor_and_init("laser.lua", "right_laser")
		...
		local target1 = self:add_actor_and_init("target.lua", "target1")
		target1.position = { 0, 0, -50 }
		target1.radius = 5
		target1:calc_model_matrix()
		
		local target2 = self:add_actor_and_init("target.lua", "target1")
		target2.position = { 10, 30, -80 }
		target2.radius = 10
		target2:calc_model_matrix()
		
		local target3 = self:add_actor_and_init("target.lua", "target1")
	...
	function scene_table.add_actor_and_init(self, script_name, actor_name)
		local interface = self.callbacks.add_actor(script_name, actor_name)
