...
class Player(sc.DelegatingActor):
	...
	def HandleAddToScene(self, args):
		...
		scene.MakeCommandAfter(
			...
			sc.IOListenerRegistration(
				True, self.id, sc.ListenerId.KEY_TOGGLE, sc.VectorUnsignedChar((ord('W'), ord('A'), ord('S'), ord('D'), ord('Q'), ord('E'),))))
		scene.MakeCommandAfter(
			scene.FrontOfCommands(),
			sc.Target(scene.FindByName("IOInterface")),
			sc.IOListenerRegistration(
	...
	def HandleMouseMovement(self, args):
		...
		self.rotation = sc.Quaternion.RotationAboutAxis(sc.AxisID.y, self.yaw) * sc.Quaternion.RotationAboutAxis(sc.AxisID.x, self.pitch)
		self.PushPose()
	...
	def HandleKeyToggle(self, args):
		if args.key in self.motion_modifiers:
			...
			location_thing = tuple(motion_modifier[1] * (1 if args.pressed else -1) if i == motion_modifier[0] else 0 for i in range(3))
			self.motion += sc.Location(*location_thing)
			mag = pow(abs(self.motion[0]) + abs(self.motion[1]) + abs(self.motion[2]), 0.5)
			if mag:
	...
	def HandleTick(self, args):
		if self.motion_scale:
			self.location += self.motion.Rotate(self.rotation) * self.motion_scale * self.move_speed * args.duration


	def PushPose(self):
		self.entity_handler.MutableCamera("player_head").SetPose(sc.Pose(self.location, self.rotation))