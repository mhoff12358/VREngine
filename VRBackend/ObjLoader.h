...
public:
	...
	static ModelGenerator CreateModelsFromFile(ID3D11Device* device, std::string filename, const OutputFormat& output_format = default_output_format);

	ObjLoader(std::string filename);

	void AddLinesToModel(ID3D11Device* device, const std::vector<std::string>& input_lines, const OutputFormat& output_format, ModelGenerator& generator, std::string part_name = "");
	ModelGenerator ParseAsMultipleModels(ID3D11Device* device, const OutputFormat& output_format);
